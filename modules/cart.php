...
$urlg = explode("?",$uri);
$color = '#'.$_GET['c1'];
$url_filename = $getUrl->get_filename($urlg[0]);
...
if ($_GET['code']=='add') 
{
	...
	if ($id and $prod_cat)
	{
		...
		$cart->add_item($scid,$id,$prod_cat,$soluong,$color);
...
<br>
		...
		$giohang="<table border='1' cellpadding='3' cellspacing='0' style='border-collapse: collapse; font-family: Arial; font-size: 10pt; color: #181818' bordercolor='#BFBFBF' width='800' bgcolor='#F3F3F3'>
		  <tr>
			...
			<td width='94' align='center'><strong>Giá</strong></td>
			<td width='82' align='center'><strong>Số lượng</strong></td>
			<td width='82' align='center'><strong>Màu</strong></td>
			<td width='102' align='center'><strong>Tổng</strong></td>
		  </tr>";
		  $no=0;
		  for ($i=0;$i<=$cont['count'];$i++)
			{
				$no++;	
				 $giohang.=" <tr>
					...
					<td align='center' bgcolor='#FFFFFF'>".$cont['quantity'][$i]."</td>
					<td align='center' bgcolor='#FFFFFF'><span style='display:block;width:25px; height:25px; border:solid 1px #CCC; background:".$cont['color'][$i]."'></span></td>
		  ...
		  <tr>
			<td align='center' bgcolor='#FFFFFF'>&nbsp;</td>
			<td colspan='3' align='right' bgcolor='#FFFFFF'><strong>Tổng tiền</strong></td>
			<td colspan='2' bgcolor='#FFFFFF' align='center'><strong>".number_format($info['grandtotal'],2,',','.')." ".$cont['donvitinh'][$i]." đ</strong></td>
		  </tr>
		</table>";
