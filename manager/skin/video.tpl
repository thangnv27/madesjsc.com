...
<form action="{action}" method="post" id="inputform" onSubmit="return check_null();" enctype="multipart/form-data">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-bordered tabletdnone">
      ...
      <tr>
        ...
        <td><span style="color:#666; font-style:italic"></span></td>
...
<tr>
        ...
        <td><strong>Hiển thị ở chân trang</strong></td>

        <td><input type="checkbox" name="showinfooter" id="showinfooter" value="1" {showinfooter} /></td>
...
function trimUrlVideo(video){
	...
	if(getHost(video)=='youtube.com' || getHost(video)=='www.youtube.com'){
	  	...
	  	if(vd){
	  		...
	  		$('#pic_avatar').html('<img src="http://img.youtube.com/vi/' + vd + '/0.jpg" width="150" /><br />');

	  	}else{

	  		$('#videourl').val(url);	

	  		$('#pic_avatar').html('<img src="http://img.youtube.com/vi/' + url + '/0.jpg" width="150" /><br />');	
