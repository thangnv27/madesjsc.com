...
//	private $data_type='news';
    ...
    public function _int() {
        global $tpl, $id, $pid;
        $this->get_cat_tree(0);
		$in = array("news","product","info","video","sitemap","contact","home");
		foreach($in as $inp){
			$this->get_cat_tree1(0,$inp);
		}
    ...
    private function showAddNew() {
        global $DB, $tpl, $lang,$tree,$tree1, $dklang, $pid, $CONFIG;
        $pid = intval($pid);
        $tpl->newBlock("AddNew");
		...
		$inpage='';
		$inpage .= '<label><input name="inpage[]" type="checkbox" value="-1">none</label>
		<label><input name="inpage[]" type="checkbox" value="-2">Tất cả các trang</label>';
		if ($tree1)
			foreach ($tree1 as $k => $v) {
				foreach ($v as $i => $j) {
					
					$inpage .='<label><input name="inpage['.$k.']" type="checkbox" value="'.$k.'">'.$j.'</label>';
				}
			}
		$tpl->assign("inpage",$inpage);
    ...
    private function showUpdate() {
        global $DB, $tpl, $lang, $dklang, $id, $dir_image, $tree, $tree1, $pid;
        $tpl->newBlock("AddNew");
        if ($_GET['p'] > 1) {
            $pa = '&p=' . $_GET['p'];
        }
        ...
        if ($rs = mysql_fetch_array($db)) {
            ...
            $tpl->assign("parentid", $info['parentid']);
			$inpag = array();
			$inpag = explode(":",$rs['inpage']);
			$inpage='';
			$inpage .= '<label><input name="inpage[]" type="checkbox" value="-1">none</label>';
			if(in_array('-2',$inpage))
				$inpage .= '<label><input checked name="inpage[]" type="checkbox" value="-2">Tất cả các trang</label>';
			else
				$inpage .= '<label><input name="inpage[]" type="checkbox" value="-2">Tất cả các trang</label>';
			if ($tree1)
				foreach ($tree1 as $k => $v) {
					foreach ($v as $i => $j) {
						if(in_array($k,$inpag)){	
							$inpage .='<label><input checked name="inpage['.$k.']" type="checkbox" value="'.$k.'">'.$j.'</label>';
						}else{
							$inpage .='<label><input name="inpage['.$k.']" type="checkbox" value="'.$k.'">'.$j.'</label>';
						}
					}
    ...
    private function showListCate($pid, $n) {
        global $DB, $tpl, $lang, $language,$site_address,$dir_path;
    ...
    private function save() {
        ...
        if ($data) {
			...
			$clsUrl->insert($data['url'],$type_info['tableitem'],$lastid,0,$data['data_type']);
