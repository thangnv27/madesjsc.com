...
class News extends cat_tree {
    ...
    private function showUpdate() {
        ...
        if ($rs = mysql_fetch_array($db)) {
            ...
            foreach ($color as $cl) {
                $colo .= '<div>
            	<span class="color-cell" style="background:' . $cl . '"><input name="color[]" class="picco" style="width:50px;" value="' . $cl . '" /></span><a href="#" class="btn-close-color" onClick="removeColor($(this)); return false;">x</a>                <div class="c"></div>
            </div>';
            }
            $tpl->assign("color", $colo);
    ...
    private function save() {
        global $DB,$DBi, $tpl, $lang, $dklang,$clsUrl;
        $data = $this->getData();
        if ($data) {
            ...
            if (intval($lastid) > 0){
              ...
              //  $search_obj->insertSearch($data, $data['id_category'], $this->id_item, $this->table, $lastid);
                ...
                Message::showMessage("success", "Kết nối tới dữ liệu không an toàn, vui lòng thử lại ! ($lastid)");
            }
        }
    }
    ...
    private function getData($update = 0, $id = 0) {
		...
		$data['modify_date'] = time() + $CONFIG['time_offset'];
