...
class Url{
	...
	public function insert($url,$table,$id_category,$id_item=0,$data_type,$alias_url=''){
		...
		$sql="INSERT INTO ".$this->url_table." (".$b['FIELD_NAMES'].") VALUES (".$b['FIELD_VALUES'].")";
		$db = $DB->query($sql);
		if($db) return true;
	...
	public function update($url,$table,$id_category,$id_item,$data_type){
		...
		$sql="UPDATE ".$this->url_table." SET ".$b." WHERE table = '".$table."' AND id_item = ".$id_item;
		$db=$DB->query($sql);
		if($db) return true;
	}
	
	public function delete($table,$id_category,$id_item){
		global $DB;
		$id_category = intval($id_category);
		$id_item = intval($id_item);
		$sql = "DELETE FROM ".$this->url_table. " WHERE dt_table = '".$table."' AND id_item = ".$id_item.' AND id_category = '.$id_category;
		$db = $DB->query($sql);
		if($db) return true;
	}
	...
	public function processUrl($url,$n=0){
		...
		if($rs = mysql_fetch_array($db)){
			...
			$url_new = $this->builUrl($url_new,$n);
		}else{
			$url_new = $url;	
		}
		return $url_new;
	...
	public function builUrl($str,$exten='',$auto=1){
		...
		if($auto==1){
			return $this->clean_url(trim($str)).$exten;
		}else{
			$str=str_replace("&nbsp;","-",$str);
			$str=str_replace("&#160;","",$str);
			$str=str_replace("&nbsp;"," ",$str);
	...
	public function getUrl($data_type='',$id_category,$id_item=0){
		global $DB,$rewrite,$dir_path,$url_max_number_category;	
		$id_category = intval($id_category);
		$id_item = intval($id_item);
		if($data_type != ''){
	...
	public function getParentUrl($id_category,$n=0){
		global $DB,$url_max_number_category;	
		$sql = "SELECT url,id_category FROM url WHERE id_category = $id_category AND id_item=0";
		$db = $DB->query($sql);
		if($rs = mysql_fetch_array($db)){
